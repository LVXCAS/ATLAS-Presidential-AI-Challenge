...
const ProfessionalChart = ({ symbol, marketData, timeframe, indicators, chartStyle, showVolumeProfile, tradingMode }) => {
  ...
  // Fetch real chart data for LIVE mode
  useEffect(() => {
    if (tradingMode === 'LIVE' && symbol) {
      const fetchRealChartData = async () => {
        try {
          const period = timeframe === '1M' ? '1d' : timeframe === '5M' ? '1d' : timeframe === '1H' ? '5d' : '1mo';
          const interval = timeframe === '1M' ? '1m' : timeframe === '5M' ? '5m' : timeframe === '1H' ? '1h' : '1d';
          
          const response = await fetch(`http://localhost:8001/api/v1/live/chart/${symbol}?period=${period}&interval=${interval}`);
          const result = await response.json();
          
          if (result.status === 'success' && result.data.data) {
            const chartData = result.data.data;
            
            setCandleData(chartData.map(candle => ({
              x: candle.time * 1000, // Convert to milliseconds
              open: candle.open,
              high: candle.high,
              low: candle.low,
              close: candle.close
            ...
            setPriceData(chartData.map(candle => ({
              x: candle.time * 1000,
              y: candle.close
            })));
            
            setVolumeData(chartData.map(candle => ({
              x: candle.time * 1000,
              y: candle.volume
            })));
          }
      ...
      fetchRealChartData();
      // Set up interval to refresh chart data
      const intervalId = setInterval(fetchRealChartData, 60000); // Update every minute
      return () => clearInterval(intervalId);
    }
  }, [symbol, timeframe, tradingMode]);
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#000' }}>
      ...
      <div style={{ 
        ...
        {/* Indicators */}
        <div style={{ display: 'flex', gap: '2px' }}>
          {['MA', 'RSI', 'MACD', 'BB', 'STOCH', 'VWAP', 'FIBR', 'S/R'].map(ind => (
            <button
              key={ind}
              onClick={() => {
        ...
        <div style={{ display: 'flex', gap: '2px', marginTop: '4px' }}>
          <span style={{ fontSize: '8px', color: '#666', paddingRight: '4px', alignSelf: 'center' }}>STYLE:</span>
          {['CANDLE', 'LINE', 'HEIKIN'].map(style => (
            <button
              key={style}
              onClick={() => setChartStyle(style)}
              style={{
                background: chartStyle === style ? '#ff9500' : '#333',
                color: chartStyle === style ? '#000' : '#ff9500',
                border: '1px solid #666',
                padding: '4px 6px',
                fontSize: '8px',
                cursor: 'pointer'
          ...
          <button
            ...
            style={{
              background: showVolumeProfile ? '#00ff00' : '#333',
              color: showVolumeProfile ? '#000' : '#00ff00',
              border: '1px solid #666',
              padding: '4px 6px',
              fontSize: '8px',
              cursor: 'pointer',
...
function App() {
  ...
  const fetchLiveMarketData = async () => {
    try {
      const symbols = Object.keys(marketData).join(',');
      const response = await fetch(`http://localhost:8001/api/v1/live/market-data?symbols=${symbols}`);
      const result = await response.json();
      ...
      if (result.status === 'success') {
        ...
        setOutput(prev => [...prev, { type: 'success', text: `LIVE DATA CONNECTED - ${Object.keys(result.data).length} SYMBOLS` }]);
      }
    } catch (error) {
      console.error('Live data fetch error:', error);
      setOutput(prev => [...prev, { type: 'error', text: 'LIVE DATA CONNECTION FAILED - FALLING BACK TO SIMULATION' }]);
    }
  };

  const fetchLiveNews = async () => {
    try {
      const response = await fetch('http://localhost:8001/api/v1/live/news?symbols=SPY,QQQ,AAPL,MSFT,NVDA');
      const result = await response.json();
      
      if (result.status === 'success') {
        setNewsData(result.data);
        setOutput(prev => [...prev, { type: 'success', text: `LIVE NEWS CONNECTED - ${result.data.length} ARTICLES` }]);
      }
    } catch (error) {
      console.error('Live news fetch error:', error);
      setOutput(prev => [...prev, { type: 'error', text: 'LIVE NEWS CONNECTION FAILED' }]);
    }
  };
    ...
    const initializeWebSocket = async () => {
      try {
        ...
        webSocketService.onConnection((connected) => {
          ...
          if (connected) {
            setConnectionError(null);
            console.log('🚀 HIVE TRADE QUANTUM TERMINAL v2.0 - FULLY OPERATIONAL!');
          } else {
            console.log('❌ WebSocket failed - Running in SIMULATION MODE');
          }
        });

        // Set up error handler
        webSocketService.onError((error) => {
          setConnectionError('WebSocket unavailable - Running in simulation mode');
          console.log('⚡ HIVE TRADE QUANTUM TERMINAL v2.0 - SIMULATION MODE ACTIVATED');
        });
        ...
        console.log(`⚡ HIVE TRADE QUANTUM TERMINAL v2.0 - ${tradingMode} MODE ACTIVATED`);
        console.log('🔋 All features functional - Real-time data active');
        ...
        const interval = setInterval(() => {
          ...
          // Update market data with mode-specific behavior
          setMarketData(prev => {
            const updated = { ...prev };
            Object.keys(updated).forEach(symbol => {
              const data = updated[symbol];
              const baseVolatility = data.iv || 0.3;
              
              // Mode-specific volatility and behavior
              let volatilityMultiplier, trendStrength;
              switch(tradingMode) {
                case 'SIMULATION':
                  volatilityMultiplier = 2.0; // High volatility for demo
                  trendStrength = 0.005;
                  break;
                case 'PAPER':
                  volatilityMultiplier = 1.2; // Realistic volatility
                  trendStrength = 0.002;
                  break;
                case 'LIVE':
                  volatilityMultiplier = 1.0; // Real market volatility
                  trendStrength = 0.001;
                  break;
                default:
              ...
              const marketTrend = Math.sin(updateCount * 0.1) * 0.1;
              const randomChange = (Math.random() - 0.5) * baseVolatility * data.price * 0.005 * volatilityMultiplier;
              const totalChange = randomChange + (marketTrend * data.price * trendStrength);
              
              updated[symbol] = {
                ...data,
                price: Math.max(0.01, data.price + totalChange),
                change: data.change + totalChange,
                changePercent: ((data.change + totalChange) / (data.price - data.change)) * 100,
                volume: data.volume + Math.floor(Math.random() * 5000 * volatilityMultiplier) + 1000
              };
            });
            return updated;
          });
          ...
          if (updateCount % 5 === 0) {
            setAgents(prev => {
              const updated = { ...prev };
              // Simulate agent performance changes
              Object.keys(updated).forEach(agentKey => {
                const agent = updated[agentKey];
                if (agent.pnl !== undefined) {
                  const change = (Math.random() - 0.48) * 1000; // Slight upward bias
                  agent.pnl = Math.max(-50000, agent.pnl + change);
                  agent.trades = agent.trades + (Math.random() > 0.7 ? 1 : 0);
                }
              });
              return updated;
            });
          ...
          // Update connection counter to show activity
          setRealtimeUpdates(prev => prev + 1);
          
          // Update AI agents performance in simulation
          if (updateCount % 4 === 0) { // Update agents every 6 seconds
            setAgents(prevAgents => {
              const updatedAgents = { ...prevAgents };
              Object.keys(updatedAgents).forEach(agentKey => {
                const agent = updatedAgents[agentKey];
                if (agent.status === 'ACTIVE' && agent.pnl !== undefined) {
                  // Simulate realistic P&L changes
                  const pnlChange = (Math.random() - 0.4) * 500; // Slight positive bias
                  const newPnL = Math.max(-50000, agent.pnl + pnlChange);
                  
                  // Update trades and accuracy
                  const newTrades = Math.random() < 0.3 ? agent.trades + 1 : agent.trades;
                  const accuracyChange = (Math.random() - 0.5) * 0.5;
                  const newAccuracy = Math.max(50, Math.min(95, agent.accuracy + accuracyChange));
                  
                  updatedAgents[agentKey] = {
                    ...agent,
                    pnl: parseFloat(newPnL.toFixed(2)),
                    trades: newTrades,
                    accuracy: parseFloat(newAccuracy.toFixed(1)),
                    winRate: agent.winRate + (Math.random() - 0.5) * 0.2
                  };
                }
              });
          ...
          // Generate realistic agent trading signals
          if (updateCount % 8 === 0 && Math.random() < 0.7) { // Generate signals occasionally
            const activeAgents = Object.entries(agents).filter(([key, agent]) => 
              agent.status === 'ACTIVE' && agent.pnl !== undefined
            );
            
            if (activeAgents.length > 0) {
              const randomAgent = activeAgents[Math.floor(Math.random() * activeAgents.length)];
              const [agentKey, agent] = randomAgent;
              ...
              console.log(`🤖 [SIMULATION] ${agent.name} generated ${signal} signal for ${symbol} @ ${confidence}% confidence`);
              
              // Update master agent with latest signal
              setAgents(prev => ({
                ...prev,
                masterAgent: {
                  ...prev.masterAgent,
                  action: `${signal}_SIGNAL`,
                  target: symbol,
                  signals: prev.masterAgent.signals + 1,
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <div style={{ color: isConnected ? '#00ff00' : '#ff0000', fontSize: '9px' }}>
                      {isConnected ? '● LIVE' : <span style={{color: '#ff9500'}}>● SIMULATION</span>}
                    </div>
