#!/usr/bin/env python3
"""
Hive Trade - Live Alpaca Connection Test (ASCII-only)
...
def test_alpaca_connection():
    ...
    if not api_key or api_key == 'your_alpaca_api_key_here':
        print("[ERROR] API keys not configured properly")
        ...
        print("3. Replace 'your_alpaca_api_key_here' with your actual API key")
        print("4. Replace 'your_alpaca_secret_key_here' with your actual secret key")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   [ERROR] Market data access failed: {response.status_code}")
            print(f"   Error: {response.text}")
        ...
        if response.status_code == 200:
            ...
            print(f"   [ERROR] Positions access failed: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            print(f"   [ERROR] Orders access failed: {response.status_code}")
        
        print()
        print("CONNECTION TEST SUMMARY:")
        print("=" * 30)
        print("[SUCCESS] Alpaca API connection successful!")
        print("[SUCCESS] Account access verified")  
        print("[SUCCESS] Market data access verified")
        print("[SUCCESS] Trading permissions confirmed")
        print()
...
def test_market_hours():
    ...
    if not is_weekday:
        print("  [CLOSED] Markets CLOSED (Weekend)")
    elif is_market_hours:
        print("  [OPEN] Markets OPEN (Regular Hours)")
    elif is_extended_hours:
        print("  [EXTENDED] Markets OPEN (Extended Hours)")  
    else:
        print("  [CLOSED] Markets CLOSED (Outside Trading Hours)")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n[ERROR] Connection issues detected. Please check your API keys.")