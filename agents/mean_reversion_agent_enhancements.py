...
class MeanReversionEnhancements:
    ...
    @staticmethod
    def calculate_dynamic_rsi_thresholds(df: pd.DataFrame, period: int = 14) -> Dict:
        """
        Calculate dynamic RSI thresholds based on historical distribution

        Instead of static 30/70, use historical percentiles:
        - Trending market: Wider thresholds (20/80)
        - Ranging market: Tighter thresholds (40/60)

        Returns:
            Dict with current_rsi, oversold_threshold, overbought_threshold
        """
        # Calculate RSI
        delta = df['close'].diff()
        gain = (delta.where(delta > 0, 0)).rolling(window=period).mean()
        ...
        # Use historical percentiles for dynamic thresholds
        rsi_history = rsi.dropna()

        if len(rsi_history) >= 60:
            # In trending markets, RSI stays elevated/depressed longer
            # Check trend strength
            returns = df['close'].pct_change()
            trend_strength = abs(returns.rolling(20).mean().iloc[-1] / returns.rolling(20).std().iloc[-1])

            if trend_strength > 0.5:  # Strong trend
                # Use wider thresholds (don't fight the trend)
                oversold = rsi_history.quantile(0.20)  # 20th percentile
                overbought = rsi_history.quantile(0.80)  # 80th percentile
            else:  # Ranging market
                # Use tighter thresholds (mean reversion works better)
                oversold = rsi_history.quantile(0.30)  # 30th percentile
                overbought = rsi_history.quantile(0.70)  # 70th percentile
        else:
