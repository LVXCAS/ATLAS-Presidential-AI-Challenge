...
async def run_acceptance_test():
    ...
    logger.info("🧪 Starting Market Data Ingestor Agent Acceptance Test")
    logger.info("=" * 70)
        ...
        if result['success']:
            ...
            if 'validation' in stats:
                ...
                if schema_valid_rate >= 0.95:
                    logger.info("✅ Schema Validation: PASSED (≥95% valid records)")
                    schema_passed = True
                elif schema_valid_rate >= 0.90:
                    logger.info("⚠️  Schema Validation: MARGINAL (90-95% valid records)")
                    schema_passed = True
                else:
                    logger.info("❌ Schema Validation: FAILED (<90% valid records)")
                    schema_passed = False
            else:
                logger.warning("⚠️  No validation statistics available")
                schema_passed = False
            ...
            # Final assessment
            logger.info("\n" + "=" * 50)
            logger.info("🏆 ACCEPTANCE TEST ASSESSMENT:")
            ...
            if result['success']:
                logger.info("✅ 1. Data ingestion completed successfully")
                criteria_passed += 1
            else:
                logger.info("❌ 1. Data ingestion failed")
            
            # Criterion 2: Schema validation
            if schema_passed:
                logger.info("✅ 2. Schema validation passed")
                criteria_passed += 1
            else:
                logger.info("❌ 2. Schema validation failed")
            
            # Criterion 3: Reasonable success rate (>80%)
            success_rate = stats.get('success_rate', 0)
            if success_rate >= 0.80:
                logger.info(f"✅ 3. Success rate acceptable ({success_rate:.1%} ≥ 80%)")
                criteria_passed += 1
            else:
                logger.info(f"❌ 3. Success rate too low ({success_rate:.1%} < 80%)")
            ...
            if records_per_second >= 1.0:
                logger.info(f"✅ 4. Performance acceptable ({records_per_second:.2f} records/sec ≥ 1.0)")
                criteria_passed += 1
            else:
                logger.info(f"❌ 4. Performance too slow ({records_per_second:.2f} records/sec < 1.0)")
            ...
            if criteria_passed == total_criteria:
                ...
                logger.info("✅ Market Data Ingestor Agent meets all requirements!")
                return True
            elif criteria_passed >= 3:
                logger.info("⚠️  ACCEPTANCE TEST: MARGINAL PASS")
                logger.info("✅ Market Data Ingestor Agent meets most requirements")
                return True
            else:
                logger.info("❌ ACCEPTANCE TEST: FAILED")
                logger.info("❌ Market Data Ingestor Agent needs improvement")
                return False
                
        else:
            logger.error("❌ ACCEPTANCE TEST: FAILED")
            logger.error("❌ Data ingestion failed completely")
            ...
            if result['failed_symbols']:
                logger.error(f"❌ All symbols failed: {result['failed_symbols']}")
            
            return False
            
    except Exception as e:
        logger.error(f"❌ ACCEPTANCE TEST: FAILED WITH EXCEPTION")
...
async def main():
    ...
    if success:
        ...
        logger.info("✅ Task 2.1 Market Data Ingestor Agent: IMPLEMENTED")
        sys.exit(0)
    else:
        logger.error("\n❌ ACCEPTANCE TEST FAILED!")
        logger.error("❌ Task 2.1 Market Data Ingestor Agent: NEEDS WORK")
        sys.exit(1)
