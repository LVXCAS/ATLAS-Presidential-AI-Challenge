...
Enhanced with OpenBB Platform integration for better data quality
"""
...
# Import OpenBB data provider for enhanced options data
try:
    from agents.openbb_data_provider import openbb_provider
    OPENBB_AVAILABLE = True
    logger = get_logger(__name__)
    logger.info("OpenBB data provider integrated with options broker")
except ImportError:
    OPENBB_AVAILABLE = False
    logger = get_logger(__name__)
    logger.warning("OpenBB data provider not available, using yfinance only")
...
class OptionsBroker:
    ...
    async def _get_options_price(self, options_symbol: str, underlying: str) -> Optional[Dict]:
        """Get current options price - Enhanced with OpenBB integration"""
        ...
        try:
            ...
            if OPENBB_AVAILABLE:
                try:
                    # Get options chain from OpenBB
                    options_data = await openbb_provider.get_options_chain(underlying)
                    ...
                    if options_data:
                        ...
                        if len(options_symbol) >= 15:
                            ...
                            try:
                                ...
                                # Select correct DataFrame (calls or puts)
                                ...
                                if not options_df.empty and 'strike' in options_df.columns:
                                    matching = options_df[options_df['strike'] == strike_price]

                                    if not matching.empty:
                                        row = matching.iloc[0]
                                        price_data = {
                                            'bid': float(row['bid']) if 'bid' in row else 0.0,
                                            'ask': float(row['ask']) if 'ask' in row else 0.0,
                                            'mid': (float(row['bid']) + float(row['ask'])) / 2 if 'bid' in row and 'ask' in row else 0.0,
                                            'volume': int(row['volume']) if 'volume' in row else 0,
                                            'openInterest': int(row['openInterest']) if 'openInterest' in row else 0,
                                            'impliedVolatility': float(row['impliedVolatility']) if 'impliedVolatility' in row else 0.25
                                        }

                                        # Cache and return
                                        self.options_prices[cache_key] = price_data
                                        self.last_price_update[cache_key] = now
                                        logger.debug(f"OpenBB: Got options price for {options_symbol}: {price_data['mid']:.2f}")
                                        return price_data
                            except Exception as e:
                                logger.debug(f"OpenBB options parsing error: {e}")
                except Exception as e:
                    logger.debug(f"OpenBB options fetch failed, falling back to yfinance: {e}")
    ...
    async def get_option_quote(self, option_symbol: str, underlying: str = None) -> Optional[Dict]:
        """
        Get current option quote (bid/ask/mark) - works in both paper and live mode

        Args:
            option_symbol: Option contract symbol (e.g., AAPL240119C00150000)
            underlying: Underlying stock symbol (optional, will parse from option_symbol)

        Returns:
            Dict with 'bid', 'ask', 'mark', 'volume', etc. or None if not found
        """
        try:
            # Extract underlying if not provided
            if not underlying:
                # Parse underlying from option symbol (first letters before date)
                import re
            ...
            # Use existing _get_options_price method
            price_data = await self._get_options_price(option_symbol, underlying)

            if price_data:
                # Add 'mark' key for compatibility (midpoint of bid/ask)
                if 'mark' not in price_data and 'mid' in price_data:
                    price_data['mark'] = price_data['mid']
                return price_data
